...
class PyTorchTrainer(Trainer):
    ...
    def save(self, lr=None, batch_size=None):
        """Save the trained model to disk."""
        self.train_data, self.val_data, self.test_data = None, None, None
        self.logger = None

        # Create a filename with learning rate and batch size
        filename_suffix = f"_lr{lr}_bs{batch_size}".replace('.', 'p') if lr and batch_size else ""
        file_name = os.path.join("models", self.name + filename_suffix + ".pkl")
